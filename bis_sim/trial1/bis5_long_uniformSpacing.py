...
def findSpineHead():
    ...
    tapered_m=raw_input('is this a tapered model')
    if tapered_m == 'yes':
       num_dend = int(raw_input('Enter the number of dendritic compartments'))
       spinStart = num_dend
    else:        
...
def findSpineHead_samedia():
    ...
    print 'location of spine start is', spinStart, tempArrayVolume
    for index, item in enumerate(tempArrayVolume):
        if float("{0:0.25f}".format(item)) < float("{0:.25f}".format(Z[spinStart].volume)):
           ...
           spinLoc.append(index+spinStart)
    return spinLoc
...
def scanDistDtGrid():
    ...
    pos = findSpineHead_samedia()
    print 'positions of inputs are', pos
        ...
        for d in drange:
            ...
            for seq in seqList:
                ...
                for j in range( numSpine ):
                    ...
                    f = f + 1
                    Z[ k ].nInit = 1
                    print 'volume of input is', Z[k].volume
                    # Z[ k ].nInit = 0.1 * j
...
#def makeChemProto( name, stimAmpl = 1, diffLength = 1e-6, ):
    ...
    rdes = rd.rdesigneur(
        ...
        moogList = [
            ['#', '1', 'dend/A', 'n', 'A n', 0, 4],
            ['#', '1', 'dend/B', 'n', 'B n', 0, 2.5],
            ['#', '1', 'dend/Ca', 'n', 'Ca n', 0.0, 1]
        ]
    )
    rdes.buildModel()
