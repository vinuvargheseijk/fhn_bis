...
def main():
    ...
    plotScatter( fig, bisTree, '20', dt, 0 )
    plotScatter( fig, bisTree, '25', dt, 1 )
    plotScatter( fig, bisTree, '30', dt, 2 )
    plotScatter( fig, bisTree, '35', dt, 3 )
    #plt.show()
    #ampl = outXmlFile[-8:].replace('.xml','').replace('_','')
    plt.savefig("diffSpacing_" + "dt-" + dt + '_tuningReversed' + '.png')
