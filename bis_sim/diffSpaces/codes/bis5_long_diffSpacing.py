...
def runTrial( diffusionLength, v, dist, blanks, preStim, postStim ):
    ...
    for i in range( numSpine ):
        data = tabs[blanks + i*stride + f*i].vector
        f = f + 1
...
def writeXML( seqDtRange, drange, labels, allSimOutput ):
    ...
    tree.write( fname + '.' + str( int(params['fnumber']) ) + '_nonUniSpac_blanksTest' + '.xml' )
...
def scanDistDtGrid():
    ...
    f1 = open('./stimPntsDiffSpacing.txt','w')
