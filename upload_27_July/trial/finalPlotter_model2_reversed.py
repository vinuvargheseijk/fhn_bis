        ...
        for distanceData in dtData.iter( "distanceData" ):
            ...
            for labelData in distanceData.iter( "labelData" ):
                ...
                if label == 'aocSum':
                    ...
                    print 'value is ', values[-1]
                    aocSumAll.append( (values[-1] - np.mean( values) ) / max( values ) )
    ax = plt.subplot( 5,2,2*0 + 2 )
    ax.tick_params( direction = 'out' )
...
def main():
    ...
    plotScatter( fig, bisTree, '5' , dt, 0 )
    plotScatter( fig, bisTree, '10', dt, 1 )
    plotScatter( fig, bisTree, '15', dt, 2 )
    plotScatter( fig, bisTree, '20', dt, 3 )
    plotScatter( fig, bisTree, '25', dt, 4 )
    #plt.show()
    #ampl = outXmlFile[-5:].replace('.xml','').replace('_','')
    plt.savefig("fhn_taperingModel-model_2_dt-" + dt + '_tuningReversed' + '.png')
