...
def runTrial( diffusionLength, v, dist, blanks, preStim, postStim, pos ):
    ...
    for i in range( numSpine ):
        ...
        data = tabs[stimPntsList[i]].vector
        f = f + 1
...
def makeStimPntsList( startPoint, stride):
	stimPntsList = [0] * numSpine
	for i in range(numSpine-2):
		stimPntsList[i] = startPoint + i*stride
	stimPntsList[3] = stimPntsList[1] + 30
	stimPntsList[4] = stimPntsList[2] + 30
	return stimPntsList
    ...
    for seqDt in seqDtRange:
        ...
        for d in drange:
            ...
            if stride == 1:
            	...
            	startPoint = 27
            stimPntsList = makeStimPntsList( startPoint,stride )
            temp = []
            slopes = []
            for seq in seqList:
                print '.',
                ...
                f = 1
                pos = range(0,len(Z),1)
                print 'position for input are', pos, len(Z)
                print 'volume are', Z[0].volume, Z[-1].volume
...
#def makeChemProto( name, stimAmpl = 1, diffLength = 1e-6, ):
   ...
   # makePassiveSoma( 'cell', params['dendLength'], params['dendDiameter'] )
    ...
    rdes = rd.rdesigneur(
        ...
        cellProto = [['./branchModel_rall.p', 'soma']],
